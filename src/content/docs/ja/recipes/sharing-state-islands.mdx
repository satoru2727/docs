...
Astroのウェブサイトを [islands architecture / partial hydration](/ja/concepts/islands) で構築する際に、 **「コンポーネント間で状態を共有したい」** という問題に直面することがあります。

ReactやVueなどのUIフレームワークは、他のコンポーネントなどの [「コンテキスト」プロバイダー](https://react.dev/learn/passing-data-deeply-with-context) の利用を奨励する場合があります。   
ただし、AstroやMarkdown内で [コンポーネントを部分的にハイドレート](/ja/guides/framework-components/#hydrating-interactive-components) する場合、これらのコンテキスト ラッパーを使用することはできません。
...
- **フレームワークに依存しません。** これにより、フレームワーク間での状態共有がシームレスになります！   
  Astroは柔軟性を重視しているため、どのフレームワークを使用しても同様の開発者体験を提供するソリューションが好きです。
...
- `.astro`ファイルや [ノンハイドレート コンポーネント](/ja/guides/framework-components/#hydrating-interactive-components) からストアに書き込んでも、 [クライアントサイドコンポーネント](/ja/reference/directives-reference/#client-directives) が受け取る値には影響しません。
...
ここで、React / Preact / Solid / Vue の例で使用した `useStore` ヘルパーの代わりに、`cartItems.get()` を呼び出していることに気づいたかと思います。   
